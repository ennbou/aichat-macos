...
public class SwiftDataManager {
    ...
    public init() {
        do {
            ...
            do {
                ...
                modelContainer = try ModelContainer(
                    for: ChatSessionModel.self, MessageModel.self,
    ...
    public func fetch<T: PersistentModel>(_ type: T.Type, predicate: Predicate<T>? = nil, sortBy: [SortDescriptor<T>]? = nil) throws -> [T] {
    ...
    public func checkDatabaseHealth() -> Bool {
        do {
            ...
            let _ = try fetch(ChatSessionModel.self)
            let _ = try fetch(MessageModel.self)
            return true
        } catch {
